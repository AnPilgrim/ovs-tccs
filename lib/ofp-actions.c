...
encode_SET_RWND(const struct ofpact_set_rwnd *setRwnd,
                    ...
                    struct ofpbuf *out)
{
    uint16_t rwnd = setRwnd->rwnd;

    put_OFPAT_SET_RWND(out, rwnd);
}
                            ...
                            struct ofpbuf *out)
{
    ...
    setRwnd = ofpact_put_SET_RWND(out);
    setRwnd->rwnd = rwnd;
...
parse_set_rwnd(char *arg, struct ofpbuf *ofpacts)
{
    ...
    setRwnd = ofpact_put_SET_RWND(ofpacts);
    setRwnd->rwnd = rwnd;
    return NULL;
}
...
{
    return parse_set_rwnd(arg, pp->ofpacts);
}

/* Used when printing info to console. */
static void
format_SET_RWND(const struct ofpact_set_rwnd *setRwnd,
                const struct ofpact_format_params *fp)
{
    /* Feel free to use e.g. colors.param,
    colors.end around parameter names */
    ds_put_format(fp->s, "setRwnd:%"PRIu16, setRwnd->rwnd);
}

/* ... */

static enum ofperr
check_SET_RWND(const struct ofpact_set_rwnd *setRwnd OVS_UNUSED,
                const struct ofpact_check_params *cp OVS_UNUSED)
{
