...
static void dict_add(struct dict *d, const char *key, int over_state, int over_count, clock_t start_time, clock_t end_time) {
    if (d->size >= 100) {
        printf("Error: Dictionary is full!\n");
        return;
    }
    struct dict_port_count *item = &d->items[d->size++];
    strncpy(item->key, key, sizeof(item->key));
    // VLOG_INFO("%s",item->key);
    item->over_state = over_state;
    item->over_count = over_count;
    item->start_time = start_time;
    item->end_time = end_time;
}

static struct dict_port_count *dict_get(struct dict *d, const char *key) {
...
static void
append_port_stat_to_tccs(struct ofport *port, struct ofp13_port_stats *ps13)
{
    ...
    ofputil_port_stats_to_tccs(&ops, ps13);
...
static void
ofputil_encode_buf_cn_send(bool flag, struct ofconn *ofconn, uint32_t currentLength, struct ofport *port){
    // VLOG_INFO("123123123123");
    struct ofpbuf *msg = ofpbuf_new(0);
    ofpbuf_reserve(msg, sizeof(struct ofp_header) + sizeof(struct ofp13_port_stats) + sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint64_t));
    struct ofp_header *oh = ofpbuf_put_uninit(msg, sizeof(struct ofp_header));
    oh->version = 0x04;
    if(flag){
        oh->type = 35;
    }else{
    ...
    oh->length = htons(sizeof(struct ofp_header) + sizeof(struct ofp13_port_stats) + sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint64_t));
    oh->xid = htonl(0);
    struct ofp13_port_stats *buf_port = ofpbuf_put_uninit(msg, sizeof(struct ofp13_port_stats));
    append_port_stat_to_tccs(port, buf_port);
    // VLOG_INFO("%lu  %lu   %lu    %lu    %u", buf_port->ps.rx_packets, buf_port->ps.tx_packets, buf_port->ps.rx_bytes, buf_port->ps.tx_dropped, buf_port->duration_sec);
    char *dst = ofpbuf_put_uninit(msg, sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint64_t));
    ovs_be16 priority = 1;
    ovs_be64 cookie = 1024;
    memcpy(dst,&currentLength,sizeof(uint32_t));
    memcpy(dst+sizeof(uint32_t),&priority,sizeof(uint16_t));
    memcpy(dst+sizeof(uint32_t)+sizeof(uint16_t)+sizeof(uint16_t),&cookie,sizeof(uint64_t));
...
handle_queue_length_request(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    //获取消息体中的值port
    uint32_t port = *(uint32_t *)(data + 8);
    // VLOG_INFO("%d", ntohl(port));
    ofp_port_t ofp_port = u32_to_ofp11(ntohl(port));
    struct ofport *fport = ofproto_get_port(ofproto, ofp_port);
    // 执行 tc -s qdisc show dev 命令，并将结果保存到管道中
    char cmd[100];
    char key[32];
    sprintf(cmd, "tc -s qdisc show dev s%lu-eth%d", datapath_id, ntohl(port));
    sprintf(key, "s%lu-eth%d", datapath_id, ntohl(port));
    fp = popen(cmd, "r");
    if (fp == NULL) {
        printf("Error: failed to execute command.\n");
        return 1;
    }
    // 从管道中读取命令执行的结果，从结果中提取队列长度currentLength
    while (fgets(buffer, BUFFER_SIZE, fp) != NULL) {
        if(strncmp(buffer, prefix, n) == 0){
            sscanf(buffer, "%*s %*s %d", &currentLength);
            // VLOG_INFO("%d", currentLength);
        }
    }
    ...
    pclose(fp);
    //发送回消息
    if(dict_get(&d, key) == NULL){
        dict_add(&d, key, 0, 0, clock(), 0);
    }
    struct dict_port_count *item = dict_get(&d, key);
    if(currentLength >= l){
        VLOG_INFO("s%lu-eth%d拥塞,队列长度为:%d", datapath_id, ntohl(port),currentLength);
        if(item->over_state == 0){
            ofputil_encode_buf_cn_send(true, ofconn, currentLength, fport);
            item->over_state = 1;
        }else{
            item->end_time = clock();
            if(item->end_time - item->start_time > rtt){
                ofputil_encode_buf_cn_send(true, ofconn, currentLength, fport);
        ...
        item->end_time = clock();
        if(item->over_state == 1 && item->end_time - item->start_time > rtt){
            item->over_count++;
            item->start_time = clock();
        }
        if(item->over_count == 3){
            ofputil_encode_buf_cn_send(false, ofconn, currentLength, fport);
            item->over_state = 0;
            item->over_count = 0;
        }
    }
